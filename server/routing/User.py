...
def register_user():
    ...
    new_user = create_new_user({
        ...
        "active_hash_expires": datetime.datetime.now() + datetime.timedelta(minutes=30),
    })
...
def activate_user():
    ...
    if datetime.datetime.now() > user.active_hash_expires:
...
def login_user():
    ...
    if not identifier or identifier == "":
        return jsonify({"success": False, "message": "No identifier provided!"}), 500

    if not password or password == "":
        return jsonify({"success": False, "message": "No password provided!"}), 500

    user = get_user_by_username_or_email(identifier, identifier)
    if not user:
        return jsonify({"success": False, "message": "No user found!"}), 500

    password_valid = verify_password(password, user.password)
    if not password_valid:
        return jsonify({"success": False, "message": "Username/Email or password invalid!"}), 401
    ...
    return jsonify({"success": True, "token": token.decode('utf-8'), "user": curr_user})
